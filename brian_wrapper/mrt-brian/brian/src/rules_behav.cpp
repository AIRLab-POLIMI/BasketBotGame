...
proposed_action_list * rules_behavior::do_actions(predicate_list * predicates, rules_line_type& rule_lines)
{
  ...
  while (i!=rules->end())
      ...
      if (p!=NULL)
	{
	  ...
	  while (y!=p->end())
	    {
			char * proposed_action_name = ((*y).second)->get_name();
	      fprintf(output, "***** DO_ACTION behavior '%s', alla riga %d, cerca di aggiungere:  %s <=> %s [ %f ]\n", behav_name, (*i)->GetLineNum(), proposed_action_name, ((*y).second)->get_label(), ((*y).second)->get_membership_value());
	      fflush(output);
		  free(proposed_action_name);
	      pal->add((*y).second);
	      y++;
	    }
	}
